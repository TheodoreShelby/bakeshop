...
export const ItemDetails = ({ cart, setCart }) => {
	...
	const handleMinus = () => {
		// get first found item from the cart array to remove
		const removedItem = cart.find(cartItem => JSON.stringify(cartItem) === JSON.stringify(item));
		//remove that item from cart
		cart.splice(cart.indexOf(removedItem), 1);
		//spread new cart into setCart()
		setCart([...cart]);
	}
